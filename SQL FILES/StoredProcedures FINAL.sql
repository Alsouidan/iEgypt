...
CREATE procedure [dbo].[Register_User] @usertype varchar (max) ,@email varchar (max) ,@password varchar (max) ,@firstname varchar (max) ,@middlename varchar (max) ,@lastname varchar (max)
...
CREATE procedure [dbo].[User_login] @email varchar (max) ,@password varchar (max) ,@user_id int OUTPUT 
as
declare 
@temp_id int
...
CREATE procedure [dbo].[Apply_New_Request] @information varchar (max), @contributor_id int, @viewer_id int
as
declare @temp_id int;
insert into notification_object default values;
...
CREATE procedure [dbo].[Rating_Original_Content] @orignal_content_id int, @rating_value float, @viewer_id int
as
...
update Original_content set rating=(select avg(rate) from Rate group by original_content_id having Original_content_id=@orignal_content_id) where id=@orignal_content_id;
GO
...
--Staff User Story #6
CREATE procedure [dbo].[Most_Requested_Content]
as
...
select * from temp t inner join content c on t.id = c.id inner join [user] u on u.id = c.contributer_id inner join Original_content o on o.id = t.id
drop table temp
...
--Staff User Story #8
CREATE procedure [dbo].[Delete_Comment2]
@viewer_id int,
@original_content_id int, 
@comment_time datetime
as
if exists(select * from Comment where viewer_id=@viewer_id and original_content_id=@original_content_id)
...
go
--Procedure we made to be used in WebForm
create procedure [dbo].[getType] @id int,@code int output
as
begin
